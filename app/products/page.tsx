...
import {
  ...
  ShoppingCart, Send, FileText, Download,
} from "lucide-react";
...
const initialProducts: Product[] = [
  ...
  { id: 4, name: "Pineapple Slices", category: "Pouches", image: "/images/pineapple.jpeg", price: 149, originalPrice: 199, rating: 4.7, weight: "30g", isSale: true, reviews: [] },
  { id: 5, name: "Apple Chips", category: "Fruit Cubes", image: "/images/apple.jpeg", price: 149, originalPrice: 199, rating: 4.8, weight: "35g", badge: "New", isSale: false, reviews: [] },
  { id: 6, name: "Guava Snacks", category: "Crispy Slices", image: "/images/guava.jpeg", price: 149, originalPrice: 199, rating: 4.6, weight: "45g", isSale: true, reviews: [] },
  { id: 7, name: "Strawberry Guac", category: "Pouches", image: "/images/strawberry.jpeg", price: 149, originalPrice: 199, rating: 4.7, weight: "40g", isSale: false, reviews: [] },
];
...
function InvoiceModal({ cart, comboCart, productsList, cartTotal, comboSummary, onClose }: {
    ...
    win.document.write(`
      ...
      <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: Arial, sans-serif; font-size: 12px; color: #1a1a1a; background: white; padding: 32px; }
        ...
        .inv-title { font-size: 28px; font-weight: 900; color: #8B3A1A; text-transform: uppercase; }
        .inv-badge { font-size: 22px; font-weight: 900; color: #1a64c8; }
        .inv-meta { font-size: 11px; color: #555; margin-top: 4px; }
        .info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 20px; }
        .info-box { background: #f8f8f8; border: 1px solid #e0e0e0; border-radius: 8px; padding: 12px; font-size: 11px; line-height: 1.7; }
        .info-box h4 { font-size: 10px; font-weight: 900; text-transform: uppercase; color: #1a64c8; border-bottom: 1px solid #ddd; padding-bottom: 4px; margin-bottom: 8px; }
        table { width: 100%; border-collapse: collapse; margin-bottom: 20px; }
        thead { background: #1a1a1a; color: white; }
        th { padding: 8px 12px; font-size: 10px; text-transform: uppercase; text-align: left; }
        th:last-child, td:last-child { text-align: right; }
        th:nth-child(3), td:nth-child(3), th:nth-child(4), td:nth-child(4) { text-align: center; }
        td { padding: 8px 12px; border-bottom: 1px solid #eee; }
        tr:nth-child(even) td { background: #fafafa; }
        .totals { display: flex; justify-content: flex-end; }
        .totals-box { width: 280px; border: 1px solid #e0e0e0; border-radius: 8px; overflow: hidden; font-size: 11px; }
        .totals-row { display: flex; justify-content: space-between; padding: 6px 12px; border-bottom: 1px solid #eee; }
        .totals-grand { display: flex; justify-content: space-between; padding: 10px 12px; background: #1a1a1a; color: white; font-size: 14px; font-weight: 900; }
        .footer { text-align: center; font-size: 10px; color: #888; border-top: 1px solid #eee; padding-top: 16px; margin-top: 20px; }
        .badge-paid { background: #d1fae5; color: #065f46; padding: 2px 8px; border-radius: 20px; font-size: 10px; font-weight: 900; }
      </style>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6">
      ...
      <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative w-full max-w-2xl bg-white rounded-2xl shadow-2xl z-10 max-h-[92vh] overflow-y-auto">
        {/* Top action bar */}
        <div className="sticky top-0 bg-white z-10 flex items-center justify-between px-5 py-3 border-b">
          ...
          <div className="flex items-center gap-2">
            <button onClick={handlePrint} className="flex items-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold px-3 py-1.5 rounded-lg cursor-pointer transition-colors">
    ...
    <div className="min-h-screen bg-[#F0EAE0] text-[#181410] relative">
      ...
      <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-40 flex items-center gap-2 bg-[#181410] text-white px-3 py-2 rounded-full shadow-2xl border border-gray-700 max-w-[95vw]">
        <button onClick={() => setIsCartDrawerOpen(true)} className="flex items-center gap-1 text-[11px] font-black text-[#FFC300] cursor-pointer hover:opacity-80">
          <ShoppingCart size={15} /><span>Cart ({cart.reduce((s, i) => s + i.quantity, 0)})</span>
        </button>
        <span className="text-gray-600">|</span>
        {/* <button onClick={() => setIsComboDrawerOpen(true)} className="flex items-center gap-1 text-[11px] font-black text-green-400 cursor-pointer hover:opacity-80">
          <PackagePlus size={15} /><span>Combo ({comboSummary.totalItems})</span>
        </button> */}
        <span className="text-gray-600">|</span>
