...
const initialProducts: Product[] = [
...

  {
    ...
    image: "/images/newPineapple.jpeg",
    price: 149,
    originalPrice: 199,
    rating: 4.9,
    weight: "35g",
...
function CountdownTimer({ targetDate }: { targetDate: string }) {
  ...
  return (
    <div className="flex items-center gap-1 bg-[#181410] text-[#FFC300] px-2 py-0.5 rounded-full text-[10px] font-mono font-bold w-fit">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6">
      ...
      <motion.div
        ...
        className="relative w-full max-w-2xl bg-white rounded-2xl shadow-2xl z-10 max-h-[92vh] overflow-y-auto"
      >
        <div className="sticky top-0 bg-white z-10 flex items-center justify-between px-5 py-3 border-b">
          <h3 className="font-black text-sm uppercase text-gray-800">
            Order Invoice
          </h3>
        ...
        <div
          ref={invoiceRef}
          className="p-6 sm:p-8 space-y-6 text-[#1a1a1a]"
        >
          <div className="inv-header flex justify-between items-start border-b-2 border-gray-900 pb-5">
            <div>
              <p className="inv-title text-2xl font-black text-[#8B3A1A] uppercase">
                {STORE_NAME}
              </p>

              <p className="inv-meta text-[11px] text-gray-500 mt-0.5">
                {STORE_ADDRESS}
              </p>

              <p className="inv-meta text-[11px] text-gray-500">
                📞 +91 87577 26925 | ✉ {STORE_EMAIL}
              </p>
            ...
            <div className="text-right">
              <p className="inv-badge text-2xl font-black text-blue-700">
                INVOICE
              </p>
              ...
              <span className="badge-paid inline-block mt-1 bg-green-100 text-green-700 text-[10px] font-black px-2 py-0.5 rounded-full">
                ✓ ORDER PLACED
              </span>
            </div>
          </div>
          ...
          <div className="info-grid grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="info-box bg-gray-50 border border-gray-200 rounded-xl p-4 text-xs leading-relaxed">
              <h4 className="font-black text-[10px] uppercase text-blue-700 border-b pb-1 mb-2">
                Sold By
              </h4>
            ...
            <div className="info-box bg-gray-50 border border-gray-200 rounded-xl p-4 text-xs leading-relaxed">
              ...
              <p className="text-gray-600 mt-2">
                Payment: <strong>COD / UPI via WhatsApp</strong>
              </p>
          ...
          <div className="overflow-hidden border border-gray-200 rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="bg-gray-900 text-white">
                <tr>
                  <th className="py-2.5 px-3 text-[10px] uppercase">
                    #
                  </th>
                  ...
                  <th className="py-2.5 px-3 text-[10px] uppercase text-center">
                    Variant
                  </th>
                  ...
                  <th className="py-2.5 px-3 text-[10px] uppercase text-right">
                    Price
                  </th>
          ...
          <div className="totals flex justify-end">
            <div className="totals-box w-full sm:w-72 border border-gray-200 rounded-xl overflow-hidden text-xs">
              ...
              {comboSummary.totalItems > 0 && (
                <div className="totals-row flex justify-between px-4 py-2 border-b border-gray-100 text-gray-600">
                  <span>
                    Combo ({comboSummary.totalItems} items)
                  </span>
              ...
              <div className="totals-grand flex justify-between px-4 py-3 bg-gray-900 text-white">
                ...
                <span className="font-mono font-black text-[#FFC300] text-sm">
                  ₹{grandWithTax}
                </span>
              </div>
            </div>
          ...
          <div className="footer text-center text-[10px] text-gray-400 border-t pt-4">
            ...
            <p>
              For delivery queries: WhatsApp +91 87577 26925 |{" "}
              {STORE_EMAIL}
            </p>
...
export default function ProductsPage() {
  ...
  const [comboCart, setComboCart] =
    useState<Record<number, number>>({});

  const [isComboDrawerOpen, setIsComboDrawerOpen] =
    useState(false);

  const [isInvoiceOpen, setIsInvoiceOpen] =
    useState(false);

  const [activeReviewProduct, setActiveReviewProduct] =
    useState<Product | null>(null);

  const [newReviewAuthor, setNewReviewAuthor] =
    useState("");
  ...
  // ─────────────────────────────────────────
  // Add To Cart
  // Coming Soon products are blocked
  // ─────────────────────────────────────────
  ...
  const handleAddToCart = (product: Product) => {
    ...
    const selectedWeight =
      selectedWeights[product.id] || product.weight;

    setCart((prev) => {
      const idx = prev.findIndex(
        (i) =>
          i.product.id === product.id &&
          i.selectedWeight === selectedWeight
      );
  ...
  const handleAddReview = (
    ...
    setProductsList((p) =>
      p.map((prod) => {
        if (prod.id !== activeReviewProduct.id)
          return prod;

        const reviews = [rev, ...prod.reviews];
  ...
  const comboSummary = useMemo(() => {
    ...
    Object.entries(comboCart).forEach(
      ([pId, qty]) => {
        const prod = productsList.find(
          (p) => p.id === Number(pId)
        );
...
>>>>>>> bd303ea16a63a9aaf06bda7e453bf50258c4eee3
    ...
    if (totalItems === 0) {
      alert(
        "Cart aur Combo dono khali hain!"
      );
      return;
    }

    const grandTotal =
      cartTotal + comboSummary.finalPrice;
    ...
    if (cart.length > 0) {
      ...
      cart.forEach((item, i) => {
        ...
        msg += `   ${item.quantity} x ₹${item.unitPrice} = *₹${
          item.quantity * item.unitPrice
        }*\n`;
      });
    ...
    if (comboSummary.totalItems > 0) {
      ...
      if (
        ...
        msg += `🎉 Combo Discount: -${comboSummary.comboBonusPercent}%\n`;
      }
  ...
  return (
    <div className="min-h-screen bg-[#F0EAE0] text-[#181410] relative">
      <section className="max-w-7xl mx-auto px-3 sm:px-6 py-4 sm:py-6">
        ...
        <div className="flex items-center gap-2 mb-4 overflow-x-auto pb-1 scrollbar-none">
          <Filter
            size={13}
            className="text-[#E8115B] shrink-0"
          />
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-full font-extrabold text-[10px] uppercase tracking-wider whitespace-nowrap transition-all cursor-pointer border ${
                selectedCategory === cat
                  ? "bg-[#181410] text-[#FFC300] border-[#181410]"
                  : "bg-white border-gray-200 text-[#181410] hover:bg-[#FFC300]"
              }`}
            >
              {cat}
            </button>
        ...
        {filteredProducts.length > 0 ? (
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2.5 sm:gap-5 pb-28">
            ...
            {filteredProducts.map((product) => {
              ...
              return (
                ...
                >
                  ...
                  {product.isComingSoon ? (
                    <div className="relative block aspect-square w-full overflow-hidden bg-[#F5EAD8]">
                      ...
                      <div className="absolute top-2 left-2 z-20">
                        <span className="flex items-center gap-1.5 bg-[#181410] text-[#FFC300] text-[9px] font-black uppercase tracking-wider px-2.5 py-1.5 rounded-full shadow-lg">
                      ...
                      <button
                        ...
                        className="absolute bottom-2 right-2 z-30 w-7 h-7 flex items-center justify-center rounded-full bg-white/90 shadow-sm hover:scale-110 transition-transform cursor-pointer"
                      >
                        <Heart
                          size={13}
                          className={
                    ...
                    <Link
                      ...
                      className="relative block aspect-square w-full overflow-hidden bg-[#F5EAD8] cursor-pointer"
                    >
                      <Image
                        src={product.image}
                        alt={product.name}
                  ...
                  <div className="flex flex-col flex-1 px-2.5 pt-2 pb-2.5 gap-1.5">
                    ...
                    {product.isComingSoon ? (
                      <h3 className="text-[13px] sm:text-[14px] font-extrabold text-[#8B2E08] leading-tight line-clamp-2">
                        {product.name}
                      </h3>
                    ) : (
                      <Link
                    ...
                    <button
                      ...
                      className={`mt-auto flex w-full items-center justify-center gap-1.5 rounded-xl py-2.5 text-[12px] font-black uppercase tracking-wider shadow-sm transition-all ${
                        product.isComingSoon
                          ? "bg-gray-300 text-gray-500 cursor-not-allowed"
                          : "bg-[#8B3A1A] text-white hover:bg-[#7A2E12] active:scale-[0.98] cursor-pointer"
                      }`}
                    >
                      {product.isComingSoon ? (
                        <>
          ...
          <div className="text-center py-16 bg-white rounded-2xl border border-gray-200">
            ...
            <button
              ...
              className="mt-4 px-6 py-2.5 bg-[#FFC300] text-[#181410] font-extrabold text-xs uppercase rounded-full"
            >
              Reset Filters
            </button>
          </div>
      ...
      <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-40 flex items-center gap-2 bg-[#181410] text-white px-3 py-2 rounded-full shadow-2xl border border-gray-700 max-w-[95vw]">
        ...
        <button
          ...
          className="flex items-center gap-1 text-[11px] font-black text-[#FFC300] cursor-pointer hover:opacity-80"
        >
        ...
        <button
          ...
          className="flex items-center gap-1 text-[11px] font-black text-blue-400 cursor-pointer hover:opacity-80"
        >
          <FileText size={14} />
        ...
        <button
          ...
          className="flex items-center gap-1 bg-[#25D366] text-white text-[11px] font-black px-3 py-1.5 rounded-full hover:bg-[#20bd5a] transition-all cursor-pointer"
        >
          <Send size={12} />
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="w-6 h-6 rounded-full bg-white flex items-center justify-center shadow-sm hover:bg-gray-200 cursor-pointer"
                        >
                          <Minus size={11} />
                        </button>
            ...
            >
              ...
              <div className="p-5 bg-gray-50 border-t space-y-2">
                ...
                <div className="text-xs font-bold space-y-1">
                  ...
                  {comboSummary.comboBonusPercent >
                    0 && (
        ...
        {activeReviewProduct && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-lg bg-white rounded-3xl p-6 shadow-2xl z-10 max-h-[85vh] flex flex-col"
            >
              ...
              <div className="overflow-y-auto my-4 space-y-3 max-h-52">
                ...
                {activeReviewProduct
                  ...
                  activeReviewProduct.reviews.map(
                    (rev) => (
                      <div
                        key={rev.id}
                        className="p-3 bg-gray-50 rounded-xl text-xs space-y-1"
                      >
              ...
              >
                ...
                <textarea
                  ...
                  className="w-full text-xs p-2.5 border rounded-lg h-20"
                  required
                />

                <button
                  type="submit"
                  className="w-full py-2 bg-[#8B3A1A] text-white font-bold text-xs uppercase rounded-lg"
                >
                  Submit Review
                </button>
