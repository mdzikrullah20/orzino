...
const Navbar = () => {
  ...
  return (
    <nav
      className={`sticky top-0 z-50 px-4 sm:px-6 transition-all duration-300 bg-[#181410] text-white ${
        scrolled
          ? "py-2 sm:py-3 shadow-xl border-b border-white/10"
          : "py-3 sm:py-4"
      }`}
    >
      <div className="max-w-6xl mx-auto flex items-center justify-between gap-4">
        ...
        <ul className="hidden md:flex items-center gap-7 text-white/90 font-bold text-[14px] uppercase tracking-wider">
          {links.map((link) => (
            <li key={link.href} className="relative group">
              <Link
                href={link.href}
                className="py-1 transition-colors duration-200 group-hover:text-[#FFC300]"
              >
                {link.label}
              </Link>
        ...
        <div ref={searchRef} className="hidden lg:block relative w-64">
          <div className="relative flex items-center">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-white/10 border border-white/20 rounded-full text-xs text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#FFC300] transition-all"
            />
            {searchQuery && (
              <button
        ...
        <div className="hidden md:block">
          <Link
            href="/products"
            className="cursor-pointer bg-[#FFC300] text-[#181410] px-5 py-2 sm:px-6 sm:py-2.5 rounded-full font-black uppercase text-[13px] sm:text-[14px] tracking-wider flex items-center gap-2 hover:bg-white hover:scale-105 active:scale-95 transition-all duration-300 shadow-lg"
          >
            <ShoppingBag size={18} />
            Order Now
          </Link>
        ...
        <button
          className="md:hidden text-white p-1 hover:text-[#FFC300] transition-colors"
          onClick={() => setOpen((o) => !o)}
      ...
      <div
        className={`md:hidden overflow-hidden transition-all duration-300 ease-out ${
          open
            ? "max-h-96 mt-3 opacity-100 border-t border-white/10 pt-3"
            : "max-h-0 opacity-0"
        }`}
      >
        {/* Mobile Search Input */}
        <div className="relative flex items-center mb-3">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-white/10 border border-white/20 rounded-full text-xs text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#FFC300]"
          />
        </div>

        {/* Mobile Links */}
        <ul className="flex flex-col gap-3 pb-3 text-white font-bold uppercase text-[14px] tracking-wider">
          {links.map((link, i) => (
            <li
              key={link.href}
            ...
            >
              <Link
                ...
                className="hover:text-[#FFC300] block py-1 transition-colors"
              >
                {link.label}
              </Link>
            </li>
          ...
          <Link
            href="/products"
            onClick={() => setOpen(false)}
            className="bg-[#FFC300] text-[#181410] px-6 py-2.5 rounded-full font-black uppercase text-[14px] tracking-wider w-full mt-2 flex items-center justify-center gap-2 hover:bg-white transition-colors shadow-lg"
          >
            <ShoppingBag size={18} />
            Order Now
          </Link>
